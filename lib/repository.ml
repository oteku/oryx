...
module Member (Connection : Caqti_lwt.CONNECTION) : MEMBER = struct
  ...
  (* module Member = struct type t = D.Member.t

     let t = let encode D.Member.{id; email; username; hash} = Ok
     (D.Email.to_string email, Option.(fold ~none:(fun _ -> "")
     username),D.Hash.to_string hash) in let decode (id, email, username, hash)
     = Ok (D.Member.{ id; email; username; hash }) in Caqti_type.(custom ~encode
     ~decode tup4(D.Uuid )) end *)
  type ('res, 'err) query_result =
    ('res, ([> Caqti_error.call_or_retrieve ] as 'err)) result Lwt.t

  (* let get_by_email_hash : (module Caqti_lwt.CONNECTION) -> email:Email.t ->
     hash:Hash.t -> (D.Member.t, 'err) query_result = *)
  ...
  (* let create = Caqti_request.exec Caqti_type.(tup3 Uuid.t Email.t Hash.t)
     "INSERT INTO \"Member\" (id, email, hash) VALUES (?, ?, ?)" *)
